"""
Doubly Linked List Implementation for Shopping Cart
Provides O(1) insertion and deletion operations with hash map lookup
"""
...
class LinkedListNode:
    """Node in the doubly linked list (shopping cart)"""
    ...
    def __repr__(self) -> str:
        return f"LinkedListNode(product_id={self.product_id}, quantity={self.quantity}, price={self.price})"


class DoublyLinkedList:
    """
    Doubly Linked List for Shopping Cart.
    
    Time Complexity: O(1) for insertions/deletions with hash map lookup
    Space Complexity: O(n) where n is number of items
    
    Use Case: Shopping cart with efficient add/remove/update operations
    """
    ...
    def __init__(self, user_id: str):
        ...
        self.size = 0
        self.product_map: Dict[int, LinkedListNode] = {}  # O(1) lookup: product_id -> node
    ...
    def add_item(self, product_id: int, quantity: int, price: float) -> None:
        ...
        # Create new node
        new_node = LinkedListNode(product_id, quantity, price)
    ...
    def get_item(self, product_id: int) -> Optional[LinkedListNode]:
        """
        ...
        Returns:
            LinkedListNode if found, None otherwise
        """
    ...
    def get_items_dict(self) -> List[Dict]:
        """
        Get all cart items as list of dictionaries (for API).
        Time Complexity: O(n)
    ...
    def __repr__(self) -> str:
        return f"DoublyLinkedList(user_id={self.user_id}, items={self.size}, total={self.get_total():.2f})"
